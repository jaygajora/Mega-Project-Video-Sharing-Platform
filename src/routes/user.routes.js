...
router.route("/register").post(registerUser);   // this will handle the POST request to /api/v1/user/register and it will execute the registerUser controller function which is defined in the user.controller.js file, this function will handle the logic for registering a user and sending the response back to the client, we will implement this function in the user.controller.js file
